def fHALF(str):
    ...
    # Create separate lists for all entities (no relationships)
    all_entities = []
    
    # Add all symptoms with their durations
    for symptom in symptoms_wout_duration_ALL[0]:
        ...
        # Add symptom with duration (no organ mapping)
        all_entities.append([symptom, duration, 'symptom'])
    
    # Add all organs found (no symptom mapping)
    unique_organs = list(set(all_organs_found))
    for organ in unique_organs:
        all_entities.append([organ, 'nil duration', 'organ'])

    # Keep durations as text (no mapping to numbers)
    # The durations are already in their original text form

    # Convert each sublist to a tuple
    for i in range(len(all_entities)):
        all_entities[i] = tuple(all_entities[i])
